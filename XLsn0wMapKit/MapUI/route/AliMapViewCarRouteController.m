...
- (void)viewDidLoad {
    ...
    [XLsn0wLoadingToast showMessage:@"正在查询"];
}
...
-(void)createAllSubViews{
   ...
   //目的地
    ...
    [_deastinationLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(kScreenWidth/2, 60));
        make.left.mas_equalTo(_startLabel).with.offset(kScreenWidth/2);
        make.top.mas_equalTo(self.view).with.offset(64);
    }];
    
    //添加连接线
    UILabel *line = [[UILabel alloc] initWithFrame:CGRectMake(kScreenWidth/2-30,91,60,3)];
   ...
   //路线规划内容
    ...
    [_routeContent sizeThatFits:CGSizeMake(kScreenWidth-2*10, MAXFLOAT)];
    ...
    [_routeContent mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(kScreenWidth-2*10, kScreenHeight-64-120));
