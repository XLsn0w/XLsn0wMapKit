...
- (void)viewDidLoad {
    ...
    [XLsn0wLoadingToast showMessage:@"正在地理编码"];
}
...
-(void)createSubViews{
    ...
    [_currentAddressLabel sizeThatFits:CGSizeMake(kScreenWidth-4*Cell_Border, MAXFLOAT)];
    [self.view addSubview:_currentAddressLabel];
    [_currentAddressLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(kScreenWidth-4*Cell_Border, 50));
    ...
    [_currentGeocodeView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(kScreenWidth-4*Cell_Border, 400));
