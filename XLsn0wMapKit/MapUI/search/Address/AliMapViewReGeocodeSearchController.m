...
- (void)viewDidLoad {
    ...
    [XLsn0wLoadingToast showMessage:@"正在地理反编码"];
}
...
-(void)createSubViews{
    ...
    [_currentLngLatLabel sizeThatFits:CGSizeMake(SCREEN_WIDTH-4*10, MAXFLOAT)];
    [self.view addSubview:_currentLngLatLabel];
    [_currentLngLatLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(SCREEN_WIDTH-4*10, 50));
    ...
    [_currentReGeocodeView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(SCREEN_WIDTH-4*10, 400));
